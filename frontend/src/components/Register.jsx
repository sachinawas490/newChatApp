...
function Register() {
  ...
  async function handleClick(e) {
    e.preventDefault();
    const user = { fullname, username, password, gender };
    ...
    <div className="w-screen h-screen flex justify-center items-center">
      <div className="h-[60%] flex flex-col bg-slate-800 w-[320px] rounded-xl p-4 border-fuchsia-600 text-slate-50 opacity-95 text-slate-150 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-40 border">
        ...
        <div className="text-yellow-200 py-3">
          Already have an Account? <button className=""><Link to={"/"}>Signin</Link></button>
          <button className="w-full mt-3 py-2 rounded-xl bg-slate-500" onClick={handleClick}>Sign Up</button>
        </div>
      </div>
    </div>
  );
